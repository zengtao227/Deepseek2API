...
func TestClaudeDirectAppliesCurrentInputFile(t *testing.T) {
	...
	if ds.uploads[0].Filename != "Deepseek2API_HISTORY.txt" {
	...
	if len(full.Messages) != 1 || !strings.Contains(full.Messages[0].Content, "Continue from the latest state in the attached Deepseek2API_HISTORY.txt context.") {
...
func TestClaudeCurrentInputFileUploadsToolsSeparately(t *testing.T) {
	...
	if ds.uploads[0].Filename != "Deepseek2API_HISTORY.txt" || ds.uploads[1].Filename != "Deepseek2API_TOOLS.txt" {
	...
	prompt, _ := ds.payload["prompt"].(string)
	if !strings.Contains(prompt, "Deepseek2API_TOOLS.txt") || !strings.Contains(prompt, "TOOL CALL FORMAT") {
